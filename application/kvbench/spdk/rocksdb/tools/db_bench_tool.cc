...
DEFINE_string(dbs, "", "Use the db with the following name.");
...
int db_bench_tool(int argc, char** argv) {
  ...
  if (!FLAGS_spdk.empty()) {
    FLAGS_env = rocksdb::NewSpdkEnv(rocksdb::Env::Default(), FLAGS_dbs, FLAGS_spdk, FLAGS_spdk_bdev, FLAGS_spdk_cache_size);
    if (FLAGS_env == NULL) {
