...
#include <atomic>
extern std::atomic_uint_fast64_t completed;

#ifdef __cplusplus
extern "C" {
#endif

    //   extern std::atomic_uint_fast64_t submitted;
    //extern std::atomic_uint_fast64_t completed;
    ...
    enum {
        ...
        /*
         * Used only for kvs
         * open an existing containter
         */
        COUCHSTORE_OPEN_FLAG_KVS = 16,
    };
    ...
    /*
    LIBCOUCHSTORE_API
    couchstore_error_t couchstore_open_db_kvs(int32_t dev_id,
                                              const char *container_name,
                                              couchstore_open_flags flags,
                                              Db **db);
    */
    couchstore_error_t couchstore_open_db_kvs(const char* dev_path, Db **db, int id);
    /*
#if defined (__BLOBFS_ROCKS_BENCH)
    LIBCOUCHSTORE_API
    couchstore_error_t couchstore_open_db_blobfs(rocksdb::Env *env,
                                                 const char *filename,
                                                 couchstore_open_flags flags,
                                                 Db **db);
#endif
