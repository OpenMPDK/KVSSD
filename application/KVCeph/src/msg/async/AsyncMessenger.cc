...
{
  //cct->kvsdbg.sendmsg_ex("async messenger::wait");
  lock.Lock();
  if (!started) {
    lock.Unlock();
    return;
  ...
  if (!stopped) {
      //cct->kvsdbg.sendmsg_ex("async messenger::wait - exit !stopped");
    stop_cond.Wait(lock);
    //cct->kvsdbg.sendmsg_ex("async messenger::wait - stopped");
  }

  lock.Unlock();

  //cct->kvsdbg.sendmsg_ex("async messenger::wait - shutting down");
