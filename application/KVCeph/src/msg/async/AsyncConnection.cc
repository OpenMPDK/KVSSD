...
// Constant to limit starting sequence numbewr to 2^31.  Nothing special about it, just a big number.  PLR
  ...
  do {
    ...
    switch (state) {
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_MESSAGE:
        {
          if (policy.throttler_messages) {
              //async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] throttle message: current %ld / max %ld\n", policy.throttler_messages->get_current(), policy.throttler_messages->get_max());
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_BYTES:
        {
          ...
          if (cur_msg_size) {
            if (policy.throttler_bytes) {
                //async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] throttle bytes: current %ld / max %ld\n", policy.throttler_bytes->get_current(), policy.throttler_bytes->get_max());
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_DISPATCH_QUEUE:
        {
          if (cur_msg_size) {
            if (!dispatch_queue->dispatch_throttler.get_or_fail(cur_msg_size)) {
                //async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] throttle dispatch queue: current %ld / max %ld\n", dispatch_queue->dispatch_throttler.get_current(), dispatch_queue->dispatch_throttler.get_max());
      ...
      case STATE_OPEN_MESSAGE_READ_FOOTER_AND_DISPATCH:
        ...
        //  async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] read the message: front %ld, middle %ld, data %ld\n",front.length(),middle.length(), data.length());
	  ...
	  ldout(async_msgr->cct, 5) << " rx " << message->get_source() << " seq "
          ...
          if (delay_state) {
            ...
            //async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] fast dispatch: message, datasize = %ld, received bytes = %ld\n", message->get_data_len(), cur_msg_size + sizeof(ceph_msg_header) + sizeof(ceph_msg_footer));
            ...
            //async_msgr->cct->kvsdbg.sendmsg_ex("[asyncmsg] dispatch: message, datasize = %ld, received bytes = %ld\n", message->get_data_len(), cur_msg_size + sizeof(ceph_msg_header) + sizeof(ceph_msg_footer));
