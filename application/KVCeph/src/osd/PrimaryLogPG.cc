...
void PrimaryLogPG::log_op_stats(OpContext *ctx)
{
  ...
  // utime_t pglock_time = ctx->op->get_pglock_end_time();
  // pglock_time -= ctx->op->get_pglock_start_time();
...
void PrimaryLogPG::op_applied(const eversion_t &applied_version)
{
  ...
  if (!g_conf->is_kvsstore) {
      assert(applied_version > last_update_applied);
      last_update_applied = applied_version;
  } else {
      //update when current applied version is more recent
      //cct->kvsdbg.sendmsg_ex("on_applied\n");
      if (last_update_applied < applied_version) {
          last_update_applied = applied_version;
      }
  }
...
// cct->kvsdbg.sendmsg_ex("on_applied - end\n");
}
...
void PrimaryLogPG::eval_repop(RepGather *repop)
{
  ...
  if (repop->all_applied && repop->all_committed) {
      ...
      if (repop_queue.front() != repop) {
          if (!repop->applies_with_commit && !g_conf->is_kvsstore) {
      ...
      if (g_conf->is_kvsstore) {
          //cct->kvsdbg.sendmsg_ex("eval_reop\n");
      ...
      //}
        //cct->kvsdbg.sendmsg_ex("eval_reop -end\n");
  }
}
