...
//#define OSD_FTRACE(ct) (ct)->kvsdbg.sendmsg_ex("%s\n", __func__)
//#define OSD_FTRACE_DONE(ct) (ct)->kvsdbg.sendmsg_ex("%s - done\n", __func__)
...
OSD::OSD(CephContext *cct_, ObjectStore *store_,
 ...
 // use these threads to work on all PG queues 
...
int OSD::init()
{
  ...
  //cct->kvsdbg.sendmsg_ex("OSD::%s, DONE\n", __func__);
  return 0;
monout:
  exit(1);

out:
  //cct->kvsdbg.sendmsg_ex("OSD::%s, FAILED\n", __func__);
...
void OSD::create_logger()
{
  ...
  osd_plb.add_u64_counter(
	...
	// #SCHED added for pglock
  osd_plb.add_time_avg(
    l_osd_pg_lock_latency_r,  "op_r_pglock_latency",
    "Latency of pglock operation for read");
  osd_plb.add_time_avg(
    l_osd_pg_lock_latency_w,  "op_w_pglock_latency",
    "Latency of pglock operation for write");
  osd_plb.add_time_avg(
    l_osd_pg_lock_latency_rw,  "op_rw_pglock_latency",
    "Latency of pglock operation for read-modify-write");

  osd_plb.add_u64_avg(
    l_osd_active_opworker_count,  "osd_active_opworker_count",
    "number of opworker threads actively working on dequeued operations");
...
int OSD::read_superblock()
{
  ...
  if (r < 0) {
    	derr << "superblock read error = " << r << dendl;
	return r;
  }
...
void OSD::handle_osd_map(MOSDMap *m)
{
  ...
  dout(10) << __func__ << " HANDLE_OSD_MAP - END " << dendl;
}
...
void OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap *m)
{
  ...
  if (!is_active()) {
    ...
    peering_wq.drain();
    dout(10) << " not yet active; waiting for peering wq to drain - done" << dendl;
  } else {
    activate_map();
  }
  ...
  if (do_shutdown) {
    ...
    dout(10) << " END: commited_osd_maps " << dendl;
}
...
/*
 * not accurate to record dequeue time here, it's actually doing executing of op
 * moved to actuall PG dequeue time at _process()
  utime_t now = ceph_clock_now();
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
  ...
  assert(NULL != sdata);
  //osd->cct->kvsdbg.sendmsg_ex("shards = %d, thread_index = %d, shard_index = %d", num_shards, thread_index, shard_index);
  ...
  utime_t now = ceph_clock_now();
  boost::optional<PGQueueable>  qi = item.second;
  boost::optional<OpRequestRef> _op = qi->maybe_get_op();
  // record actual dequeue time
  if (_op) {
    (*_op)->set_dequeued_time(now);
    utime_t latency = now - (*_op)->get_req()->get_recv_stamp();
...
ghobject_t OSD::get_osdmap_pobject_name(epoch_t epoch) {
  #ifdef USE_KVSKEYTYPE
  return kvs_get_osdmap_pobject_name(epoch);
  #else
  char foo[20];
...
ghobject_t OSD::get_inc_osdmap_pobject_name(epoch_t epoch) {
  #ifdef USE_KVSKEYTYPE
  return kvs_get_inc_osdmap_pobject_name(epoch);
  #else
...
ghobject_t OSD::make_snapmapper_oid() {
  #ifdef USE_KVSKEYTYPE
  return kvs_make_snapmapper_oid();
  #else
...
ghobject_t OSD::make_pg_log_oid(spg_t pg) {

  #ifdef USE_KVSKEYTYPE
  return kvs_make_pg_log_oid(pg);
  #else
  stringstream ss;
  ss << "pglog_" << pg;
  string s;
...
ghobject_t OSD::make_pg_biginfo_oid(spg_t pg) {
  #ifdef USE_KVSKEYTYPE
  return kvs_make_pg_biginfo_oid(pg);
  #else
  stringstream ss;
  ss << "pginfo_" << pg;
  string s;
...
ghobject_t OSD::make_infos_oid() {

  #ifdef USE_KVSKEYTYPE
  return kvs_make_infos_oid();
  #else
