...
//#define KVS_DEBUG2(ct) cerr << TEXT_RED << "CEPH-OSD::" << __func__  << ":" << __LINE__ << ", counter =" << DEBUG_COUNTER++ << TEXT_NORMAL << std::endl;
#define KVS_DEBUG2(ct)
...
#endif
{
  ...
  std::string cluster_msgr_type = g_conf->ms_cluster_type.empty() ? g_conf->get_val<std::string>("ms_type")
  ...
  if (store_type != "kvsstore" && global_init_preload_erasure_code(g_ceph_context) < 0)
    return -1;
#endif
