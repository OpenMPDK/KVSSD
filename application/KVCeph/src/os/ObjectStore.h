...
public:
  ...
  virtual bool has_onode_prefetcher() { return false; }
  virtual void prefetch_onode(const coll_t& cid, const ghobject_t* oid) {}
...
public:
  // support io_queue per thread.
   //virtual void *get_ioqueue(bool shared = false) { return 0; }
   //virtual int queue_transactions(void *ioqueue, Sequencer *osr, vector<Transaction>& tls, TrackedOpRef op = TrackedOpRef(), ThreadPool::TPHandle *handle = NULL) { return queue_transactions(osr, tls, op, handle); }
};
