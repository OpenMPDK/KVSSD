  ...
  uint32_t op_flags)
{
  ...
  if (!c->exists)
    return -ENOENT;
  //cct->kvsdbg.sendmsg_ex("Memstore read: oid=%s\n", stringify(oid).c_str());
  ObjectRef o = c->get_object(oid);
  if (!o)
    return -ENOENT;
  //cct->kvsdbg.sendmsg_ex("1\n");
  if (offset >= o->get_size())
    return 0;
  size_t l = len;
  //cct->kvsdbg.sendmsg_ex("2\n");
  if (l == 0 && offset == 0)  // note: len == 0 means read the entire object
    l = o->get_size();
  else if (offset + l > o->get_size())
    l = o->get_size() - offset;
  bl.clear();
  //cct->kvsdbg.sendmsg_ex("3 offset = %ld, size = %ld\n", offset, l);
  int ret = o->read(offset, l, bl);
 //cct->kvsdbg.sendmsg_ex("4 read done? %d\n", ret);
  return ret;
}
    ...
    case Transaction::OP_OMAP_SETKEYS:
      {
        coll_t cid = i.get_cid(op->cid);
        ghobject_t oid = i.get_oid(op->oid);
        //cct->kvsdbg.sendmsg_ex("MemStore::OP_OMAP_SETKEYS  cid=%s, oid=%s\n", stringify(cid).c_str(), stringify(oid).c_str());
        bufferlist aset_bl;
        i.decode_attrset_bl(&aset_bl);
        //cct->kvsdbg.sendmsg_ex("MemStore::OP_OMAP_SETKEYS  - check\n");
