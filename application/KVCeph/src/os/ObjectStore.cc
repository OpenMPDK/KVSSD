...
unsigned ObjectStore::apply_transactions(Sequencer *osr,
					 ...
					 Context *ondisk)
{
  //cct->kvsdbg.sendmsg_ex("OSD::Apply_Transaction - start ");
  // use op pool
  Cond my_cond;
  ...
  //cct->kvsdbg.sendmsg_ex("OSD::Apply_Transaction - queue transaction - start");
  queue_transactions(osr, tls, onreadable, ondisk);

  //cct->kvsdbg.sendmsg_ex("OSD::Apply_Transaction - queue transaction - done ");
  my_lock.Lock();
  while (!done)
    my_cond.Wait(my_lock);
  my_lock.Unlock();

  //cct->kvsdbg.sendmsg_ex("OSD::Apply_Transaction - finsished");
  return r;
}
