...
void KvsReadContext::read_coll(const char *name, const int namelen)
{
    ...
    this->value = KvsMemPool::Alloc_value(KVS_OBJECT_MAX_SIZE);
    construct_collkey(key, name, namelen);

    std::bitset<32> writeKey(*(uint32_t*)key->key);
    derr << "READ_COLL KEY = " << writeKey << dendl;
}
...
kv_result KvsReadContext::read_wait(bufferlist &bl) {
    ...
    if (retcode == KV_SUCCESS && value != 0) {
        ...
        /*
        bufferptr p = buffer::create_page_aligned(value->length);
        char *buf = p.c_str();
        memcpy(buf, value->value, value->length);
        bl.append(p);
        */
    }
