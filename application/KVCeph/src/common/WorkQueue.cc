...
void ThreadPool::worker(WorkThread *wt)
{
  ...
  while (!_stop) {
	...
	if (item) {
      ...
      //cct->kvsdbg.sendmsg_ex("WorkerThread - thread index = %d", wt->thread_index);
      TPHandle tp_handle(cct, hb, wq->timeout_interval, wq->suicide_interval, wt->thread_index);
...
void ThreadPool::start_threads()
{
  ...
  while (_threads.size() < _num_threads) {
    WorkThread *wt = new WorkThread(this, thread_index);
