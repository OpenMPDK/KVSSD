...
tpch9 <- read.csv("C:\\docs\\work\\myrocks\\q9_trx_0913.csv", header = TRUE)
tpch9_trx <- tpch9
tpch9$operation = as.factor(tpch9$operation)
levels(tpch9$operation)
run_type = "TPCH Q9"
...
plot1 <-barplot(height=op_group$total, beside = TRUE, col = 1:7,  las=2,
                ylab = "Frequency",
                xaxt = "n",
                main = "TCPH Q9 Operation Distribution",
            cex.names = 0.75)

text(x = plot1, y = par("usr")[3] - 1, srt = 35,
     adj = 1, labels = op_group$operation, xpd = TRUE)

##############
dev.off()

procfunc <- function(data, run_type, phase) {
    setwd("C:\\docs\\work\\myrocks\\q9_plot")
    ...
    put_val_title = sprintf("%s %s Phase PUT Histogram of Value Sizes", run_type, phase)

    get_key_title = sprintf("%s %s Phase GET Histogram of Key Sizes", run_type, phase)
    get_val_title = sprintf("%s %s Phase GET Histogram of Value Sizes", run_type, phase)
