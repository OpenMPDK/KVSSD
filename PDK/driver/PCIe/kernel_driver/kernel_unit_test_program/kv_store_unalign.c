...
void usage(void)
{
	printf("[Usage] kv_store_unalign -d device_path -k key_string [-v value_pattern -l offset -s value_size -z space_id -i(idempotent flag)]\n");
}
...
int main(int argc, char *argv[])
{
	...
	if (value_len) {
        int alloc_len = ((value_len % 4) ? value_len + ( 4 - (value_len % 4)) : value_len) + 3;
	...
	ret = nvme_kv_store(space_id, fd, nsid, key, key_len, buf + 1, (int)value_len, (int)offset, option); 
	if (ret) {
