...
#if 1
static int __nvme_init_iod(struct request *rq, struct nvme_dev *dev, bool kv)
{
...
static int nvme_init_iod_for_kv(struct request *rq, struct nvme_dev *dev)
{
		...
		struct nvme_command *cmnd)
{
    ...
    if (param->kv_data_sg_ptr) {
    	...
    	if (!nvme_setup_prps(dev, req, param->kv_data_len))
	    	goto out_unmap;
			 ...
			 const struct blk_mq_queue_data *bd)
{
	...
	if (is_kv_cmd(cmnd.common.opcode)) {
        ...
        if (req->special) {
            ...
            cqe.commmand_id = cmnd.common.command_id;
            cqe.status = 0;
		    if (is_kv_retrieve_cmd(cmnd.common.opcode)) {
			    cqe.result = cmnd.common.cdw[10] << 2;
            }
