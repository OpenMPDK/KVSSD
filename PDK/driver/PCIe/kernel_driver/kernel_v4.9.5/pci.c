...
static int nvme_init_iod_for_kv(struct request *rq, unsigned size,
		struct nvme_dev *dev)
{
    return __nvme_init_iod(rq, size, dev, true);
}
			 ...
			 const struct blk_mq_queue_data *bd)
{
    ...
    if (b_kv_cmd) {
		struct nvme_iod *iod = blk_mq_rq_to_pdu(req);
        struct nvme_io_param *param = &iod->param;
        map_len = param->kv_data_len;
	    ret = nvme_init_iod_for_kv(req, map_len, dev);
	    if (ret)
		    goto out;
		iod->kv_cmd = 1;
    } else {
    ...
    if (!b_kv_cmd) {
#ifdef REPORT_NON_KV
		pr_err("nvme_queue_rq: non-kv-opcode (%02x) ns_id(%d)\n", cmnd.common.opcode, ((ns)? ns->ns_id : 0));
#endif //#ifdef REPORT_NON_KV
    }
...
#if 0
#if defined(REPORT_NON_KV) || defined(KV_NVME_TRACE)
    {
		struct nvme_command cmd;
	    ...
	    {
#ifdef KV_NVME_TRACE
		    pr_err("nvme_comple_rq: opcode (%02x) nsid(%d) request (%d).\n", cmd.common.opcode, cmd.common.nsid, req->errors);
#endif
	    } else {
#ifdef REPORT_NON_KV
		    pr_err("nvme_comple_rq: non kv opcode (%02x) nsid(%d) request (%d).\n", cmd.common.opcode, cmd.common.nsid, req->errors);
#endif
        }
    }
#endif
